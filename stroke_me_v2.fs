// "Stroke me: Stage 2" by @vilkastakan
...
void main() {
    ...
    if (u_time < 60.0) {
        cd = (gl_FragCoord.xy) / (u_time/2.0); //+ u_mouse/50.0;
    }
    float len = length(vec2(cd.x, cd.y));
    cd.x = cd.x + sin(cd.y + sin(len*0.1)) + u_time*noise(cd/3.0)/7000.0 + 50.0;
    ...
    gl_FragColor = vec4(
        ...
        sin(cd.x*0.1*noise(cd))+blackout, 
